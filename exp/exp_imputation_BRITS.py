...
def parse_delta(masks, seq_len, feature_num):
    ...
    for h in range(seq_len):
        if h == 0:
            deltas.append(np.zeros((batch_size,feature_num)))
        else:
            deltas.append(np.ones((batch_size,feature_num)) + (1 - masks[:,h,:]) * deltas[-1])
    output = np.asarray(deltas).astype("float32")
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                inputs_tuple = {
                    ...
                    "forward": {"X": inp, "missing_mask": mask, "deltas": torch.from_numpy(
                        parse_delta(mask.detach().cpu().numpy(), self.args.seq_len, self.args.enc_in)).to(
                        batch_x.device)},
                    "backward": {
                        "X": inp.flip(1).detach().clone(),
                        "missing_mask": mask.flip(1).detach().clone(),
                        "deltas": torch.from_numpy(
                            parse_delta(mask.flip(1).detach().cpu().numpy(), self.args.seq_len, self.args.enc_in)).to(
                            batch_x.device),
                    },
                }

                results = self.model(inputs_tuple, "train")
                # loss = criterion(outputs, batch_x)

                outputs = results["imputed_data"]
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                inputs_tuple = {
                    "X_holdout": inp,
                    "indicating_mask": mask,
                    "forward": {"X": inp, "missing_mask": mask, "deltas": torch.from_numpy(parse_delta(mask.detach().cpu().numpy(),self.args.seq_len,self.args.enc_in)).to(batch_x.device)},
                    "backward": {
                        "X": inp.flip(1).detach().clone(),
                        "missing_mask": mask.flip(1).detach().clone(),
                        "deltas": torch.from_numpy(parse_delta(mask.flip(1).detach().cpu().numpy(),self.args.seq_len,self.args.enc_in)).to(batch_x.device),
                    },
                }
                ...
                for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                    ...
                    inputs_tuple = {
                        ...
                        "backward": {
                            ...
                            "missing_mask": mask.flip(1).detach().clone(),
                            "deltas": torch.from_numpy(
                                parse_delta(mask.flip(1).detach().cpu().numpy(), self.args.seq_len,
                                            self.args.enc_in)).to(
                                batch_x.device),
                        },
                    }
                    ...
                    if i % 200 == 0:
                        ...
                        # filled_zero_pred = pred[0, :, -1] * (1 - mask[0, :, -1])
                        # true_zero = true[0,:,-1]*(1 - mask[0, :, -1])
                        # mask_visual(true_zero, filled_zero_pred, os.path.join(folder_path, str(i) + 'mask_rate{}.pdf'.format(mask_rate)))
